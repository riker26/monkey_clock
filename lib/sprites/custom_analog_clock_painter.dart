...
class CustomAnalogClockPainter extends CustomPainter {
  DateTime datetime;
  ...
  @override
  bool shouldRepaint(CustomAnalogClockPainter oldDelegate) {
  ...
  void _paintClockHands(Canvas canvas, Size size, double scaleFactor) {
    ...
    double longHandLength =
        (r - (p * scaleFactor)) * minuteHandLengthMultiplier;
    double secondHandLength =
        (r - (p * scaleFactor)) * secondHandLengthMultiplier;
    double shortHandLength =
        (r - (p + 36.0) * scaleFactor) * hourHandLengthMultiplier;
